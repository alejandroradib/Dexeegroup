...
import { assignQuestions, getAssessmentByType, isExpired, processOralAttempt, scorePsychometricAttempt, scoreWrittenAttempt, type AssessmentType } from "@/server/services/assessments";
...
const audioSchema = z.object({ attempt_id: z.uuid(), question_id: z.uuid(), path: z.string().min(5).max(300), duration_seconds: z.number().min(0).max(600) });
...
export async function startAttempt(type: string): Promise<Result<{ attemptId: string }>> {
  ...
  const { data, error } = await supabase.from("assessment_attempts").insert({ assessment_id: assessment.id, candidate_id: user.id }).select("*").single();
  if (error) {
    if (error.message.includes("attempt_already_open")) {
      const { data: open } = await supabase.from("assessment_attempts").select("id").eq("assessment_id", assessment.id).eq("candidate_id", user.id).eq("status", "in_progress").maybeSingle();
  ...
  try {
    ...
    await createAdminClient().from("assessment_attempts").update({ status: "expired" }).eq("id", data.id);
    return err(ERR.generic);
  }
...
export async function saveAssessmentAnswers(attemptId: string, input: unknown): Promise<Result<null>> {
  const user = await requireCandidate();
  if (!user) return err(ERR.unauthorized);
...
export async function recordTabLeave(attemptId: string): Promise<Result<null>> {
  ...
  const { data: attempt } = await admin.from("assessment_attempts").select("id, integrity, status").eq("id", attemptId).eq("candidate_id", user.id).maybeSingle();
  ...
  await admin.from("assessment_attempts").update({ integrity: { ...integrity, tab_leaves: (integrity.tab_leaves ?? 0) + 1 } as Json }).eq("id", attemptId);
  return ok(null);
}
...
export async function confirmAudioUpload(input: unknown): Promise<Result<null>> {
  ...
  if (!parsed.success) return err(ERR.validation);
  if (!parsed.data.path.startsWith(`attempts/${parsed.data.attempt_id}/`)) return err(ERR.forbidden);
  const supabase = await createClient();
  const { error } = await supabase
    .from("assessment_answers")
    .upsert({ attempt_id: parsed.data.attempt_id, question_id: parsed.data.question_id, audio_path: parsed.data.path, audio_duration_seconds: parsed.data.duration_seconds, transcript: null, ai_feedback: null }, { onConflict: "attempt_id,question_id" });
...
export async function submitAttempt(attemptId: string): Promise<Result<{ status: string }>> {
  ...
  if (!user) return err(ERR.unauthorized);
  const admin = createAdminClient();
  const { data: attempt } = await admin.from("assessment_attempts").select("*, assessments (*)").eq("id", attemptId).eq("candidate_id", user.id).maybeSingle();
  ...
  const integrity = { ...((attempt.integrity as Record<string, unknown> | null) ?? {}), submitted_after_expiry: expired } as Json;
  ...
  const { data: final } = await admin.from("assessment_attempts").select("status").eq("id", attemptId).single();
  revalidatePath("/[locale]/candidate", "layout");
  return ok({ status: final?.status ?? "submitted" });
}

export async function getAudioPlaybackUrl(attemptId: string, questionId: string): Promise<Result<{ url: string }>> {
  const user = await getSessionUser();
  if (!user) return err(ERR.unauthorized);
  const admin = createAdminClient();
  const { data: attempt } = await admin.from("assessment_attempts").select("candidate_id").eq("id", attemptId).maybeSingle();
  if (!attempt) return err(ERR.notFound);
  if (user.role !== "admin" && attempt.candidate_id !== user.id) return err(ERR.forbidden);
  const { data: answer } = await admin.from("assessment_answers").select("audio_path").eq("attempt_id", attemptId).eq("question_id", questionId).maybeSingle();
  if (!answer?.audio_path) return err(ERR.notFound);
  const { data, error } = await admin.storage.from("assessment-audio").createSignedUrl(answer.audio_path, 600);
