...
export async function getCompanyDashboard(companyId: string): Promise<CompanyDashboard> {
  ...
  const [jobs, applications] = await Promise.all([
    ...
    supabase
      .from("applications")
      .select(
        "id, status, created_at, contact_requested_at, contact_released, jobs!inner (company_id)",
      )
  ...
  return {
    ...
    contactRequestsPending: appRows.filter((a) => a.contact_requested_at && !a.contact_released)
      .length,
  };
}
