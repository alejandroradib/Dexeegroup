...
export async function getCurrentCandidateProfile(userId: string): Promise<CandidateProfile | null> {
  ...
  const [contact, experience, education] = await Promise.all([
    ...
    supabase.from("candidate_experience").select("*").eq("candidate_id", userId).order("sort_order").order("start_date", { ascending: false }),
    supabase.from("candidate_education").select("*").eq("candidate_id", userId).order("end_year", { ascending: false }),
  ]);
  return { candidate, contact: contact.data, experience: experience.data ?? [], education: education.data ?? [] };
}

export type CandidateApplication = Tables["applications"]["Row"] & {
  job: Pick<PublicJob, "id" | "title" | "slug" | "company_name" | "confidential_company" | "contract_type"> | null;
...
export async function listCandidateApplications(userId: string): Promise<CandidateApplication[]> {
  const supabase = await createClient();
  const { data: applications } = await supabase.from("applications").select("*").eq("candidate_id", userId).order("created_at", { ascending: false });
  const rows = applications ?? [];
  if (rows.length === 0) return [];
  const jobIds = rows.map((a) => a.job_id);
  const [{ data: jobs }, { data: events }] = await Promise.all([
    supabase.from("public_jobs").select("id, title, slug, company_name, confidential_company, contract_type").in("id", jobIds),
    supabase.from("application_events").select("*").in("application_id", rows.map((a) => a.id)).order("created_at", { ascending: true }),
  ]);
  const jobById = new Map((jobs ?? []).map((j) => [j.id, j]));
  return rows.map((a) => ({ ...a, job: jobById.get(a.job_id) ?? null, events: (events ?? []).filter((e) => e.application_id === a.id) }));
}

export async function getCandidateApplicationForJob(userId: string, jobId: string) {
  const supabase = await createClient();
  const { data } = await supabase.from("applications").select("id, status, created_at").eq("candidate_id", userId).eq("job_id", jobId).maybeSingle();
  return data;
}
...
export async function listRecommendedJobs(candidate: Candidate, limit = 6): Promise<PublicJob[]> {
  const supabase = await createClient();
  let query = supabase.from("public_jobs").select("*").order("published_at", { ascending: false }).limit(60);
  ...
  return (data ?? [])
    .filter((job) => {
      if (job.english_level_required && rank !== null && CEFR_RANK[job.english_level_required] > rank) return false;
      if (job.english_level_required && rank === null) return false;
      if (candidate.desired_salary_min_usd !== null && job.salary_max_usd !== null && job.salary_max_usd < candidate.desired_salary_min_usd) return false;
      return true;
    })
    .slice(0, limit);
}
...
export async function getAssessmentHub(userId: string): Promise<AssessmentHubItem[]> {
  ...
  const [{ data: assessments }, { data: attempts }] = await Promise.all([
    ...
    supabase.from("assessment_attempts").select("*").eq("candidate_id", userId).order("created_at", { ascending: false }),
  ]);
  ...
  for (const assessment of assessments ?? []) {
    ...
    const { data: next } = await supabase.rpc("assessment_cooldown_ok", { target_assessment_id: assessment.id, target_candidate_id: userId });
    const nextAllowedAt = (next as string | null) ?? null;
    const now = Date.now();
    const remainingMinutes = open?.expires_at ? Math.max(0, Math.round((new Date(open.expires_at).getTime() - now) / 60000)) : null;
...
export async function listDataRequests(userId: string) {
  const supabase = await createClient();
  const { data } = await supabase.from("data_requests").select("*").eq("candidate_id", userId).order("created_at", { ascending: false });
  return data ?? [];
}
