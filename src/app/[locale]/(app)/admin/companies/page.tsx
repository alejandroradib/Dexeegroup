...
export default async function AdminCompaniesPage({
  ...
  const status =
    ...
    (COMPANY_STATUSES as readonly string[]).includes(query.status)
      ? (query.status as Database["public"]["Enums"]["company_status"])
      : undefined;
  ...
  const hrefFor = (p: number) => {
    ...
    return `/admin/companies${s.size ? `?${s}` : ""}`;
  };
  return (
    <>
      ...
      {rows.length === 0 ? (
        <EmptyState icon={BuildingIcon} title={t("empty")} />
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableRow key={c.id}>
                <TableCell>
                  ...
                  <p className="text-muted-foreground text-xs">
                    {c.sector ? te(`sector.${c.sector}`) : ""}
                  </p>
                </TableCell>
                <TableCell>
                  <p>{c.owner?.full_name}</p>
                  <p className="text-muted-foreground text-xs">{c.owner?.email}</p>
                </TableCell>
                <TableCell>
                  <StatusChip kind="company" status={c.status} />
                </TableCell>
