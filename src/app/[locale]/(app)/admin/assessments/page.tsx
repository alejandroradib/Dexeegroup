...
export default async function AdminAssessmentsPage({
  params,
}: PageProps<"/[locale]/admin/assessments">) {
  await pageLocale(params);
  const [t, te, format, queue] = await Promise.all([
  ...
  return (
    <>
      ...
      {queue.length === 0 ? (
        <EmptyState icon={ClipboardCheckIcon} title={t("empty")} description={t("emptyBody")} />
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {queue.map((a) => (
              <TableRow key={a.id}>
                ...
                <TableCell>
                  {a.submitted_at ? format.dateTime(new Date(a.submitted_at), "short") : "—"}
                </TableCell>
                <TableCell>
                  <Button asChild size="sm" variant="outline">
                    <Link href={`/admin/assessments/attempts/${a.id}`}>{t("open")}</Link>
                  </Button>
                </TableCell>
              </TableRow>
            ))}
