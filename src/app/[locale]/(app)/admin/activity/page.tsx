...
export default async function AdminActivityPage({ params, searchParams }: PageProps<"/[locale]/admin/activity">) {
  ...
  const hrefFor = (p: number) => { const s = new URLSearchParams(); if (action) s.set("action", action); if (entity) s.set("entity", entity); if (p > 1) s.set("page", String(p)); return `/admin/activity${s.size ? `?${s}` : ""}`; };
  return (
    <>
      <PageHeader title={t("title")} />
      <FilterBar>
        <FilterField label={t("action")}><input name="action" defaultValue={action ?? ""} placeholder="company." className={filterInputClass} /></FilterField>
        <FilterField label={t("entity")}>
          <select name="entity" defaultValue={entity ?? ""} className={filterInputClass}>
            <option value="">—</option>
            {["company", "job", "candidate", "application", "placement", "assessment", "assessment_attempt", "profile", "data_request"].map((e) => <option key={e} value={e}>{e}</option>)}
          </select>
        </FilterField>
      </FilterBar>
      {rows.length === 0 ? <EmptyState icon={ActivityIcon} title={t("empty")} /> : (
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.id}>
                <TableCell className="whitespace-nowrap">{format.dateTime(new Date(r.created_at), { dateStyle: "short", timeStyle: "short" })}</TableCell>
                <TableCell>{r.profiles?.full_name ?? r.profiles?.email}</TableCell>
                <TableCell className="font-medium text-navy">{r.action}</TableCell>
                <TableCell className="text-xs text-muted-foreground">{r.entity_type}{r.entity_id ? ` · ${r.entity_id.slice(0, 8)}` : ""}</TableCell>
                <TableCell className="max-w-[320px] truncate font-mono text-xs text-muted-foreground">{JSON.stringify(r.metadata)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
