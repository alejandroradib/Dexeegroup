...
export default async function AdminTeamPage({ params }: PageProps<"/[locale]/admin/team">) {
  ...
  const [t, format, admins, user] = await Promise.all([
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <Table>
          ...
          <TableBody>
            {admins.map((a) => (
              <TableRow key={a.id}>
                <TableCell className="text-navy font-medium">{a.full_name}</TableCell>
                <TableCell>{a.email}</TableCell>
                <TableCell>{format.dateTime(new Date(a.created_at), "short")}</TableCell>
                <TableCell>
                  {a.id !== user?.id ? <DeactivateAdminButton userId={a.id} /> : null}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
