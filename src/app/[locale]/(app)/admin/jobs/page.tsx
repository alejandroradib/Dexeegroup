...
export default async function AdminJobsPage({
  ...
  const status =
    typeof query.status === "string" && (JOB_STATUSES as readonly string[]).includes(query.status)
      ? (query.status as Database["public"]["Enums"]["job_status"])
      : undefined;
  const { rows, total } = await listAdminJobs(
    { queue: tab === "queue", status, q },
    pageRange(page),
  );
    ...
    <>
      ...
      {rows.length === 0 ? (
        <EmptyState
          icon={BriefcaseIcon}
          title={t("empty")}
          description={tab === "queue" ? t("emptyBody") : undefined}
        />
      ) : (
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {rows.map((job) => (
              <TableRow key={job.id}>
                ...
                <TableCell>
                  <Link href={`/admin/companies/${job.companies?.id}`} className="hover:underline">
                    {job.companies?.name}
                  </Link>
                  {job.companies?.status !== "verified" ? (
                    <p className="text-warning text-xs">
                      {te(`company_status.${job.companies?.status ?? "pending"}`)}
                    </p>
                  ) : null}
                </TableCell>
                <TableCell>
