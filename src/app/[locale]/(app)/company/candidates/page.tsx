...
export default async function CompanyCandidatesPage({
  params,
  searchParams,
}: PageProps<"/[locale]/company/candidates">) {
  ...
  const status =
    ...
    APPLICATION_STATUSES.includes(query.status as ApplicationStatus)
  ...
  const [{ rows, total }, jobs] = await Promise.all([
    listCompanyApplicants(company!.id, { status, jobId, q }, pageRange(page)),
  ...
  return (
    <>
      ...
      <form
        method="get"
        className="border-border mb-4 grid gap-3 rounded-[12px] border bg-white p-4 sm:grid-cols-[1fr_200px_200px_auto]"
      >
        <input
          name="q"
          defaultValue={q ?? ""}
      ...
      <ApplicantsTable
        rows={rows}
        emptyState={<EmptyState icon={UsersIcon} title={t("empty")} description={t("emptyBody")} />}
      />
