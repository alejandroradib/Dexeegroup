...
export async function generateMetadata({ params }: PageProps<"/[locale]/jobs/[slug]">): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: { canonical: `/${locale}/jobs/${slug}`, languages: { en: `/en/jobs/${slug}`, es: `/es/jobs/${slug}` } },
...
export default async function JobPage({ params }: PageProps<"/[locale]/jobs/[slug]">) {
  ...
  const env = publicEnv();
  const logoUrl = job.company_logo_path ? `${env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/logos/${job.company_logo_path}` : null;
  const jsonLd = buildJobPostingJsonLd(job, { siteUrl: env.NEXT_PUBLIC_SITE_URL, locale, publicLogoUrl: logoUrl });
  ...
  return (
    <div className="container-marketing py-12">
      ...
      <Button asChild variant="link" className="mb-6 px-0">
        <Link href="/jobs"><ArrowLeftIcon /> {t("backToJobs")}</Link>
      </Button>
      <JobDetail
        job={job}
        applyAction={
          <Button asChild variant="accent" className="w-full" size="lg">
            <Link href={applyHref}>{user?.role === "candidate" ? t("applyCta") : t("applySignUp")}</Link>
          </Button>
        }
      />
      {similar.length > 0 ? (
        <section className="mt-16">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {similar.map((s) => <JobCard key={s.id} job={s} />)}
          </div>
        </section>
      ) : null}
    </div>
