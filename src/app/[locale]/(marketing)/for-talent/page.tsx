...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/for-talent">): Promise<Metadata> {
    ...
    <>
        ...
        <div className="max-w-3xl">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Button asChild variant="accent" size="lg">
              <Link href="/sign-up/candidate">{t("ctaTitle")}</Link>
            </Button>
            <Button
              asChild
              size="lg"
            ...
            >
              <Link href="/jobs">{tc("footer.jobs")}</Link>
            </Button>
          </div>
        </div>
      </Section>
      ...
      <Section tone="mist" id="assessments">
        ...
        <div className="border-border mt-10 rounded-[12px] border bg-white p-6">
          ...
          <ChevronList
            className="mt-4"
            items={[t("assessmentsReceive1"), t("assessmentsReceive2"), t("assessmentsReceive3")]}
          />
          <p className="text-muted-foreground mt-4 text-xs">{tc("consentNotice")}</p>
        </div>
      </Section>
      <Section>
        <SectionTitle title={t("howTitle")} />
        <Steps
          steps={[1, 2, 3, 4].map((n) => ({ label: String(n), body: t(`how${n}` as "how1") }))}
        />
      </Section>
      <Section tone="mist">
