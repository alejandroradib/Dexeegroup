      ...
      {steps.map((step, index) => {
        ...
        const content = (
          <>
            ...
            <span className={cn("text-sm", active ? "font-semibold text-navy" : "text-slate", "hidden sm:inline")}>{step.label}</span>
          </>
        );
        return (
          <li key={step.id} className="flex items-center gap-2" aria-current={active ? "step" : undefined}>
            {onSelect && done ? (
              <button type="button" onClick={() => onSelect(index)} className="flex items-center gap-2 rounded-[8px] hover:underline">
                {content}
              </button>
            ) : (
              <span className="flex items-center gap-2">{content}</span>
            )}
            {index < steps.length - 1 ? <span className="hidden h-px w-6 bg-border sm:block" aria-hidden /> : null}
          </li>
        );
      })}
    </ol>
