...
export function ReleaseContactButton({
  applicationId,
  company,
}: {
  ...
  return (
    <ConfirmButton
      ...
      variant="accent"
      title={t("release")}
      description={t("releaseConfirm", { company })}
      onConfirm={async () => {
        const r = await releaseContact(applicationId);
        toast({
          title: r.ok ? t("released") : tc("errors.generic"),
...
export function StatusSelect({ applicationId, status }: { applicationId: string; status: Status }) {
  ...
  return (
    <select
      ...
      onChange={(e) =>
        start(async () => {
          const r = await adminChangeApplicationStatus({
            application_id: applicationId,
            status: e.target.value as Status,
          });
          toast({
            title: r.ok ? t("statusChanged") : tc("errors.generic"),
            variant: r.ok ? "success" : "danger",
          });
          router.refresh();
        })
      }
      className="border-input h-8 rounded-[8px] border bg-white px-2 text-xs"
    >
