    ...
    <div className="flex flex-wrap gap-2">
      ...
      {company.status !== "suspended" ? (
        <ConfirmButton
          ...
          onConfirm={async () => {
            const r = await setCompanyStatus(company.id, "suspended");
            notify(r.ok, t("suspended"));
            router.refresh();
          }}
        >
          {t("suspend")}
        ...
        <Button
          ...
          onClick={() =>
            start(async () => {
              const r = await setCompanyStatus(company.id, "verified");
              notify(r.ok, t("reinstated"));
              router.refresh();
            })
          }
        >
            ...
            <ul className="grid gap-2">
              {pendingJobs.map((job) => (
                <li key={job.id}>
                  <label
                    htmlFor={`pub-${job.id}`}
                    className="border-border flex items-center gap-3 rounded-[10px] border p-3 text-sm"
                  >
                    <Checkbox
                      id={`pub-${job.id}`}
                      ...
                      onCheckedChange={(c) =>
                        setSelected((prev) =>
                          c ? [...prev, job.id] : prev.filter((id) => id !== job.id),
                        )
                      }
                    />
                    {job.title}
              ...
              onClick={() =>
                start(async () => {
                  const r = await verifyCompany({
                    ...
                    publish_job_ids: selected,
                  });
                  notify(r.ok, t("verified"));
                  setOpen(false);
                  router.refresh();
                })
              }
