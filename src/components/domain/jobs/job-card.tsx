...
export function salaryLabel(
  job: Pick<PublicJob, "salary_min_usd" | "salary_max_usd" | "show_salary">,
  t: ReturnType<typeof useTranslations<"common.labels">>,
  format: ReturnType<typeof useFormatter>,
) {
  if (!job.show_salary || (job.salary_min_usd === null && job.salary_max_usd === null))
    return t("salaryHidden");
...
export function JobCard({ job, hrefBase = "/jobs" }: { job: PublicJob; hrefBase?: string }) {
  ...
  return (
    <article className="border-border flex flex-col gap-3 rounded-[12px] border bg-white p-5 transition-shadow hover:shadow-md">
      ...
      <p className="font-heading text-navy text-base font-bold">{salaryLabel(job, tl, format)}</p>
      <dl className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1 text-sm">
        {job.contract_type ? (
          <div className="flex items-center gap-1.5">
            ...
            <dd>{te(`contract_type.${job.contract_type}`)}</dd>
          </div>
        ) : null}
        {job.employment_type ? (
          <div className="flex items-center gap-1.5">
            <ClockIcon className="size-4" aria-hidden />
            <dd>{te(`employment_type.${job.employment_type}`)}</dd>
          </div>
        ) : null}
        {job.english_level_required ? (
          <div className="flex items-center gap-1.5">
            <GlobeIcon className="size-4" aria-hidden />
            <dd>{t("englishRequired", { level: job.english_level_required })}</dd>
          </div>
        ) : null}
      </dl>
      ...
      {job.published_at ? (
        <p className="text-muted-foreground text-xs">
          {t("publishedOn", { date: format.dateTime(new Date(job.published_at), "short") })}
        </p>
      ) : null}
    </article>
  );
