...
export function AudioRecorder({
  ...
  const startRecording = useCallback(() => {
    ...
    const rec = new MediaRecorder(
      stream,
      mimeType ? { mimeType, audioBitsPerSecond: 48000 } : undefined,
    );
    chunks.current = [];
  ...
  return (
    <div className="border-border grid gap-3 rounded-[12px] border bg-white p-5">
      {state === "idle" ? (
        <Button variant="accent" onClick={startPreparation} disabled={disabled}>
          <MicIcon /> {t("prepare")}
        </Button>
      ) : null}
      {state === "preparing" ? (
        <div className="grid gap-3">
          <p className="text-navy text-sm font-medium">{t("prepare")}</p>
          <p
            className="font-heading text-navy text-4xl font-extrabold tabular-nums"
            aria-live="polite"
          >
            {t("prepareLeft", { seconds })}
          </p>
      ...
      {state === "recording" ? (
        <div className="grid gap-3">
          ...
          <p
            ...
            aria-live="polite"
          >
            {t("recordingLeft", { seconds })}
          </p>
          <div className="bg-mist h-2 w-full rounded-full">
            <div
              className={cn("h-2 rounded-full", elapsed >= minSeconds ? "bg-green" : "bg-navy")}
              style={{ width: `${(elapsed / maxSeconds) * 100}%` }}
            />
          </div>
          <p className="text-muted-foreground text-xs">{t("minReached")}</p>
          <Button variant="destructive" onClick={stopRecording} disabled={elapsed < minSeconds}>
            <SquareIcon /> {t("stop")}
          </Button>
        </div>
      ) : null}
