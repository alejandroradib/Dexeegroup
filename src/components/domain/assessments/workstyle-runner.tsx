...
export function WorkstyleRunner({
  ...
  const initial = useMemo(
    () =>
      Object.fromEntries(
        saved.map((a) => [
          a.question_id,
          { selected_option: a.selected_option, likert_value: a.likert_value } as AnswerValue,
        ]),
      ),
    [saved],
  );
  ...
  const scaleLabels =
    (likert[0]?.options as { scale?: { labels_en: string[]; labels_es: string[] } } | null)
      ?.scale ?? null;
  const labels = scaleLabels
    ? locale === "es"
      ? scaleLabels.labels_es
      : scaleLabels.labels_en
    : ["1", "2", "3", "4", "5"];
  ...
  return (
    <div className="grid gap-6">
      <Progress
        ...
        aria-label={t("unanswered", { count: unanswered })}
      />
      ...
      <ol className="grid gap-4">
        {current.map((q) => {
          ...
          const prompt =
            locale === "es"
              ? ((q.options as { text_es?: string; prompt_es?: string } | null)?.text_es ??
                (q.options as { prompt_es?: string } | null)?.prompt_es ??
                q.prompt)
              : q.prompt;
          if (isLikert) {
            return (
              <li key={q.id} className="border-border rounded-[12px] border bg-white p-4">
                <fieldset>
                  <legend className="text-navy text-sm font-medium">{prompt}</legend>
                  <div className="mt-3 grid grid-cols-5 gap-1">
                    {[1, 2, 3, 4, 5].map((v) => (
                      <button
                        key={v}
                        type="button"
                        onClick={() => setAnswer(q.id, { likert_value: v })}
                        aria-pressed={answers[q.id]?.likert_value === v}
                        aria-label={labels[v - 1]}
                        className={cn(
                          "rounded-[8px] border px-1 py-2 text-xs sm:text-[11px]",
                          answers[q.id]?.likert_value === v
                            ? "border-navy bg-navy text-white"
                            : "border-border text-muted-foreground hover:bg-mist bg-white",
                        )}
                      >
          ...
          const choices =
            (q.options as { choices?: { id: string; text: string; text_es?: string }[] } | null)
              ?.choices ?? [];
          return (
            <li key={q.id} className="border-border rounded-[12px] border bg-white p-4">
              <fieldset>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {page < pages.length - 1 ? (
          ...
          <ConfirmButton
            ...
            onConfirm={submit}
            disabled={pending || unanswered > 0 || saveState === "closed"}
          >
