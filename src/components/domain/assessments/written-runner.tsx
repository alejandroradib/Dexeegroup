...
function buildMcqRows(questions: PublicQuestion[]): McqRow[] {
  ...
  for (const q of questions) {
    ...
    rows.push({
      question: q,
      passage,
      showPassage: Boolean(passage && passage.id !== lastPassage),
    });
...
export function WrittenRunner({
  ...
  const { answers, setAnswer, flush, saveState } = useAttemptAnswers(
    attempt.id,
    initialAnswers(saved),
  );
  const [expired, setExpired] = useState(() =>
    attempt.expires_at ? new Date(attempt.expires_at).getTime() <= Date.now() : false,
  );
  ...
  function submit() {
    start(async () => {
      ...
      if (result.ok) {
        ...
        toast({
          title: result.error === "conflict" ? t("attemptClosed") : tc("errors.generic"),
          variant: "danger",
        });
    });
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_260px]">
      <div className="grid gap-6">
        ...
        <ol className="grid gap-6">
          {mcq.map(({ question: q, passage, showPassage }, index) => {
            const choices = ((q.options as { choices?: Choice[] } | null)?.choices ??
              []) as Choice[];
            return (
              <li key={q.id} className="grid gap-3">
                {showPassage && passage ? (
                  <article className="border-border bg-mist/60 rounded-[12px] border p-5">
                ...
                <fieldset className="border-border rounded-[12px] border bg-white p-5">
                  <legend className="sr-only">
                    {t("question", { current: index + 1, total: mcq.length })}
                  </legend>
                  <p className="text-muted-foreground text-xs">
                    {t("question", { current: index + 1, total: mcq.length })} ·{" "}
                    {t(`section.${q.section as "grammar"}`)}
                  </p>
                  <p className="text-navy mt-1 text-base font-medium">{q.prompt}</p>
                  <RadioGroup
                    className="mt-3"
                    value={answers[q.id]?.selected_option ?? ""}
                    onValueChange={(v) => setAnswer(q.id, { selected_option: v })}
                    disabled={saveState === "closed"}
                  >
                    {choices.map((c) => (
                      <label
                        key={c.id}
                        htmlFor={`${q.id}-${c.id}`}
                        className="border-border has-[[data-state=checked]]:border-navy has-[[data-state=checked]]:bg-mist flex cursor-pointer items-start gap-3 rounded-[10px] border p-3 text-sm"
                      >
        ...
        {writing ? (
          <section className="border-border rounded-[12px] border bg-white p-5">
            ...
            <p className="text-navy mt-2 text-base font-medium">{writing.prompt}</p>
            <p className="text-muted-foreground mt-1 text-xs">
              {t("wordTarget", {
                min: writingOpts.min_words ?? 150,
                max: writingOpts.max_words ?? 200,
              })}
            </p>
            ...
            <Textarea
              ...
              className="mt-3"
              value={writingText}
              onChange={(e) => setAnswer(writing.id, { answer_text: e.target.value })}
              disabled={expired || saveState === "closed"}
              maxLength={6000}
            />
      ...
      <aside className="grid gap-3 self-start lg:sticky lg:top-24">
        ...
        <div className="border-border rounded-[12px] border bg-white p-4 text-sm">
          <p className="text-navy font-medium">{t("unanswered", { count: unanswered })}</p>
          <p className="text-muted-foreground mt-1 text-xs" aria-live="polite">
            {saveState === "saving" ? t("saving") : saveState === "saved" ? t("saved") : ""}
          </p>
          <p className="text-muted-foreground mt-2 text-xs">{t("tabLeaveNotice")}</p>
          <ConfirmButton
            variant="accent"
            ...
            onConfirm={submit}
            disabled={pending || saveState === "closed"}
          >
