...
export function Section({ children, className, tone = "white", id }: { children: ReactNode; className?: string; tone?: "white" | "mist" | "navy"; id?: string }) {
  return (
    <section
      id={id}
      className={cn(
...
export function FeatureCard({ icon: Icon, title, body }: { icon?: LucideIcon; title: string; body: string }) {
  return (
    <div className="rounded-[12px] border border-border bg-white p-6">
      {Icon ? (
        <span className="mb-4 flex size-10 items-center justify-center rounded-[10px] bg-mint text-navy">
  ...
  return (
    <ol className="grid gap-6 md:grid-cols-3 lg:grid-cols-4">
      {steps.map((step, index) => (
        <li key={step.label} className="relative rounded-[12px] border border-border bg-white p-6">
          <span className="text-xs font-semibold tracking-wide text-deep-green uppercase">{step.label}</span>
          {step.title ? <h3 className="mt-2 text-lg">{step.title}</h3> : null}
          <p className="mt-2 text-sm">{step.body}</p>
          {index < steps.length - 1 ? (
            <ChevronRightIcon className="absolute top-1/2 -right-4 hidden size-6 -translate-y-1/2 text-green lg:block" aria-hidden />
          ) : null}
        </li>
      ))}
    </ol>
...
export function Metric({ value, label }: { value: string; label: string }) {
  return (
    <div className="rounded-[12px] bg-white/5 p-6 ring-1 ring-white/10">
      <p className="font-heading text-4xl font-extrabold text-green">{value}</p>
