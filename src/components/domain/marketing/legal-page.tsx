...
export async function LegalPage({ doc, locale, title }: { doc: LegalDocument; locale: Locale; title: string }) {
  ...
  return (
    <div className="container-marketing max-w-3xl py-16">
      ...
      <p className="mt-2 text-sm text-muted-foreground">{t("updated", { date: format.dateTime(new Date(LEGAL_UPDATED), "long") })}</p>
      <Alert variant="warning" className="mt-6">{t("placeholderNotice")}</Alert>
