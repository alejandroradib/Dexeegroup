...
function Card({
  ...
  return (
    <li
      ...
      className={cn(
        "border-border rounded-[12px] border bg-white p-3 shadow-sm",
    ...
    >
      <div className="flex items-start gap-2">
        <button
          type="button"
          className="text-muted-foreground hover:bg-mist mt-0.5 cursor-grab touch-none rounded p-0.5"
        ...
        >
          ...
          <p className="text-muted-foreground line-clamp-2 text-xs">{c?.headline}</p>
        </button>
      </div>
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        {c?.years_experience !== null && c?.years_experience !== undefined ? (
          <Badge variant="outline">
            {tc("labels.yearsShort", { count: Math.round(c.years_experience) })}
          </Badge>
        ) : null}
        <CefrBadge
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-7 gap-1 px-2 text-xs">
              {t("moveTo")} <ChevronDownIcon className="size-3" />
            </Button>
          </DropdownMenuTrigger>
...
function Column({
  ...
  return (
    <section
      ref={setNodeRef}
      className={cn(
        "bg-mist/80 flex min-h-[200px] w-[260px] shrink-0 flex-col rounded-[12px] p-2",
        isOver && "ring-green ring-2",
      )}
      aria-label={te(status)}
    >
      <header className="flex items-center justify-between px-1 pb-2">
        <h3 className="text-navy text-xs font-semibold tracking-wide uppercase">{te(status)}</h3>
        <span className="text-muted-foreground rounded-full bg-white px-2 text-xs">{count}</span>
      </header>
...
export function KanbanBoard({ cards: initial, initialOpen = null }: Props) {
  ...
  return (
    <>
      <p className="text-muted-foreground mb-3 text-xs">{t("dragHint")}</p>
      <DndContext
        sensors={sensors}
      ...
      >
              ...
              <Column key={status} status={status} count={items.length}>
                {items.map((card) => (
                  <Card
                    ...
                    card={card}
                    onOpen={() => setOpen(card.application.id)}
                    onMove={(s) => move(card.application.id, s)}
                  />
                ))}
              </Column>
            );
        ...
        <DragOverlay>
          {activeCard ? (
            <ul>
              <Card card={activeCard} onOpen={() => undefined} onMove={() => undefined} dragging />
            </ul>
          ) : null}
        </DragOverlay>
      </DndContext>
        ...
        {showWithdrawn && withdrawn.length > 0 ? (
          <ul className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {withdrawn.map((card) => (
              <li
                key={card.application.id}
                className="border-border rounded-[12px] border bg-white p-3 text-sm opacity-70"
              >
                <button
                  type="button"
                  onClick={() => setOpen(card.application.id)}
                  className="text-left"
                >
                  <p className="text-navy font-medium">
                    {card.candidate
                      ? `${card.candidate.first_name} ${card.candidate.last_initial ?? ""}.`
                      : "—"}
                  </p>
                  <p className="text-muted-foreground text-xs">{card.candidate?.headline}</p>
                </button>
              </li>
            ))}
          </ul>
