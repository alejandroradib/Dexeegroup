...
export function LogoUploader({
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="border-border bg-mist flex size-20 items-center justify-center overflow-hidden rounded-[12px] border">
        {preview ? (
          <Image
            src={preview}
            alt=""
          ...
          onChange={(e) => {
            ...
            start(async () => {
              const result = await uploadViaSignedUrl({
                bucket: "logos",
                file,
                path: `companies/${companyId}/logo-${Date.now()}.${file.name.split(".").pop() ?? "png"}`,
              });
              if (result.ok) {
