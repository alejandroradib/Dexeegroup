...
export function ProfileView({ profile, email }: { profile: CandidateProfile; email: string }) {
  ...
  const hasBadges = Boolean(
    c.english_verified_level || c.english_written_level || c.psychometric_completed_at,
  );

  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
      <div className="grid gap-6">
        <div className="border-border flex flex-wrap items-center justify-between gap-3 rounded-[12px] border bg-white p-4">
          <div className="flex items-center gap-3">
            <Switch id="preview" checked={preview} onCheckedChange={setPreview} />
            <Label htmlFor="preview" className="flex items-center gap-2 font-normal">
              <EyeIcon className="size-4" aria-hidden /> {t("preview")}
            </Label>
          </div>
          <Button asChild variant="outline" size="sm">
            <Link href="/candidate/onboarding">{t("edit")}</Link>
          </Button>
        </div>
        {preview ? <Alert variant="info">{t("previewNote")}</Alert> : null}
        <article className="border-border rounded-[12px] border bg-white p-6">
          <h2 className="text-2xl">{displayName}</h2>
          <p className="text-muted-foreground mt-1">{c.headline}</p>
          <div className="text-muted-foreground mt-3 flex flex-wrap items-center gap-2 text-sm">
            ...
            {c.years_experience !== null ? (
              <span>{tc("labels.years", { count: Math.round(c.years_experience) })}</span>
            ) : null}
            {c.role_family ? (
              <Badge variant="secondary">{te(`role_family.${c.role_family}`)}</Badge>
            ) : null}
            <CefrBadge
          ...
          {c.summary ? (
            <section className="mt-6">
              <h3 className="text-navy text-sm font-semibold">{t("summary")}</h3>
          ...
          <section className="mt-6">
            <h3 className="text-navy text-sm font-semibold">{t("preferences")}</h3>
            <dl className="mt-2 grid gap-2 text-sm sm:grid-cols-2">
              {c.desired_roles.length > 0 ? (
                <div>
                  <dt className="text-muted-foreground">{t("desiredRoles")}</dt>
                  <dd>{c.desired_roles.join(", ")}</dd>
                </div>
              ) : null}
              {c.desired_salary_min_usd !== null ? (
                <div>
              ...
              {c.availability ? (
                <div>
                  ...
                  <dd>{te(`availability.${c.availability}`)}</dd>
                </div>
              ) : null}
              {c.preferred_contract_types.length > 0 ? (
                <div>
                  <dt className="text-muted-foreground">{t("contractTypes")}</dt>
                  <dd>
                    {c.preferred_contract_types.map((ct) => te(`contract_type.${ct}`)).join(", ")}
                  </dd>
                </div>
              ) : null}
              {c.english_self_level ? (
                <div>
                  ...
                  <dd>{te(`cefr_level.${c.english_self_level}`)}</dd>
                </div>
              ) : null}
            </dl>
          </section>
      ...
      <aside className="grid gap-4 lg:self-start">
        <section className="border-border rounded-[12px] border bg-white p-5">
          <h3 className="text-navy flex items-center gap-2 text-sm font-semibold">
            {preview ? <LockIcon className="size-4" aria-hidden /> : null}
            {t("contact")}
          </h3>
          {preview ? (
            <p className="text-muted-foreground mt-2 text-sm">{t("contactPrivate")}</p>
          ) : (
                ...
                <div>
                  ...
                  <dd className="truncate">
                    <a
                      ...
                      rel="noreferrer"
                    >
                      {profile.contact.linkedin_url.replace(/^https?:\/\/(www\.)?/, "")}
                    </a>
                  </dd>
                </div>
              ) : null}
                ...
                <div>
                  ...
                  <dd className="truncate">
                    <a
                      ...
                      rel="noreferrer"
                    >
                      {profile.contact.portfolio_url.replace(/^https?:\/\/(www\.)?/, "")}
                    </a>
                  </dd>
                </div>
              ) : null}
        ...
        <section className="border-border rounded-[12px] border bg-white p-5">
          <h3 className="text-navy text-sm font-semibold">{t("badges")}</h3>
          {hasBadges ? (
            ...
            <p className="text-muted-foreground mt-2 text-sm">{t("noBadges")}</p>
          )}
          <p className="text-muted-foreground mt-3 text-xs">{tc("consentNotice")}</p>
        </section>
      </aside>
    </div>
  );
