...
export function EducationStep({
  ...
  const form = useForm<EducationInput>({
    ...
    defaultValues: { institution: "", degree: "", field: "" },
  });
  ...
  const save = form.handleSubmit((values) =>
    start(async () => {
      ...
      if (result.ok) {
        setEditing(false);
        form.reset({ institution: "", degree: "", field: "" });
        router.refresh();
    ...
    <div className="grid gap-5">
      ...
      <ul className="grid gap-3">
        {rows.map((row) => (
          ...
          >
            <div>
              <p className="text-navy font-medium">{row.institution}</p>
      ...
      {editing ? (
        <form
          ...
          noValidate
        >
          <FormField id="edu_institution" label={t("institution")} error={e.institution?.message}>
            <Input id="edu_institution" {...form.register("institution")} />
          </FormField>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField id="edu_degree" label={t("degree")} optional={tc("labels.optional")}>
              <Input id="edu_degree" {...form.register("degree")} />
            </FormField>
            <FormField id="edu_field" label={t("field")} optional={tc("labels.optional")}>
              <Input id="edu_field" {...form.register("field")} />
            </FormField>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField
