...
export function ResumeStep({
  ...
  function onFile(file: File) {
    ...
    if (file.type !== "application/pdf") {
      setError(tv("fileType"));
      return;
    }
    if (file.size > MAX_BYTES) {
      setError(tv("fileSize"));
      return;
    }
    start(async () => {
      const upload = await uploadViaSignedUrl({
        bucket: "resumes",
        file,
        path: `candidates/${candidateId}/resume.pdf`,
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="border-border flex flex-col gap-3 rounded-[12px] border border-dashed p-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <span className="bg-mist text-navy flex size-10 items-center justify-center rounded-[10px]">
            <FileTextIcon className="size-5" aria-hidden />
          </span>
          <p className="text-navy text-sm font-medium">{hasResume ? t("current") : t("none")}</p>
        </div>
      ...
      <div className="flex justify-between">
        ...
        <Button type="button" variant="accent" onClick={onFinish}>
          {finishLabel}
        </Button>
      </div>
    </div>
