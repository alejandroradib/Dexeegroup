...
export function AssessmentReportPdf({
  ...
  const dateText = attempt.validated_at
    ? new Date(attempt.validated_at).toLocaleDateString(locale === "es" ? "es-CO" : "en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.tagline}>
              Nearshore talent & back-office partner for U.S. companies
            </Text>
          </View>
        ...
        {type !== "psychometric" && type !== "disc" ? (
          <View style={styles.hero}>
            ...
            {type === "english_oral" ? (
              <Text style={styles.heroLabel}>{labels.verified}</Text>
            ) : null}
          </View>
        ) : null}
        {written ? (
          <>
            <View style={styles.section}>
              ...
              <View style={styles.row}>
                <Text>
                  {written.mcq.correct} / {written.mcq.total}
                </Text>
                <Text style={styles.muted}>{written.mcq.level}</Text>
              </View>
            </View>
            {written.writing ? (
              <View style={styles.section}>
                ...
                <View style={styles.row}>
                  <Text>{written.writing.total} / 20</Text>
                  <Text style={styles.muted}>{written.writing.level}</Text>
                </View>
                <Text style={[styles.h2, { marginTop: 8 }]}>{labels.feedback}</Text>
                {written.writing.feedback.map((f, i) => (
        ...
        {workstyle ? (
          <>
            <View style={styles.section}>
              ...
              {Object.values(workstyle.factors).map((f) => (
                <View key={f.label.en} style={{ marginBottom: 6 }}>
                  <View style={styles.row}>
                    <Text style={{ fontFamily: "Helvetica-Bold" }}>{f.label[locale]}</Text>
            ...
            <View style={styles.section}>
              ...
              <Text>
                {workstyle.sjt.score} / {workstyle.sjt.total}. {workstyle.sjt.summary[locale]}
              </Text>
            </View>
            <View style={styles.section}>
              <Text style={styles.h2}>{labels.strengths}</Text>
              {workstyle.strengths[locale].map((s) => (
