...
export function monthlyMargin(input: {
...
export function marginPercent(input: {
  monthly_bill_rate_usd: number;
  monthly_salary_usd: number;
}): number | null {
...
export function totalMonthlyMargin(
  rows: { monthly_bill_rate_usd: number; monthly_salary_usd: number; status: "active" | "ended" }[],
): number {
