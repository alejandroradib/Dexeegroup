...
export const verifyCompanySchema = z.object({
  company_id: z.uuid(),
  publish_job_ids: z.array(z.uuid()).max(50),
});
...
export const candidateTagsSchema = z.object({
  candidate_id: z.uuid(),
  tags: z.array(z.string().trim().min(1).max(40)).max(20),
});
export const dexeeNoteSchema = z.object({
  candidate_id: z.uuid(),
  application_id: z.uuid().optional(),
  body: z.string().trim().min(2, "tooShort").max(2000, "tooLong"),
});
...
export const rejectAttemptSchema = z.object({
  attempt_id: z.uuid(),
  comment: z.string().trim().min(3, "tooShort").max(1000, "tooLong"),
});
export const assessmentConfigSchema = z.object({
  ...
  time_limit_minutes: z.number().int().min(1).max(1440).nullable(),
});
