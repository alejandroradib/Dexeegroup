  ...
  return [
    {
      key: "identity",
      weight: 15,
      done: filled(input.first_name) && filled(input.last_name) && filled(input.headline),
    },
    ...
    { key: "english", weight: 10, done: input.english_self_level !== null },
    {
      key: "compensation",
      weight: 10,
      done: input.desired_salary_min_usd !== null && input.availability !== null,
    },
...
export function computeCompleteness(input: CompletenessInput): number {
  const score = completenessChecklist(input).reduce(
