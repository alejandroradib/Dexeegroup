...
## 18. Conversational practice interview (2026-09-22)

Replaces the six-question written runner of DECISIONS 17 with a turn-by-turn interview. Same table (`mock_interviews`), same cooldown (30 days), same one-open-interview rule and same report scale (four dimensions 0–5, overall 0–20, bands developing / solid / strong).

- **Interviewer.** An AI interviewer (Anthropic, `interviewer.v1`) plays the hiring manager of the job, or a Dexee recruiter in the generic mode. One question at a time, short turns, one follow-up when an answer is vague, no evaluation during the interview. It covers background, one measurable achievement, two role-specific questions from the brief, remote collaboration and motivation, then closes and sets `done`. Hard caps: 10 candidate replies per interview, 2,000 characters per reply, 40 replies per candidate per hour, two hours from start.
- **Tied to a job.** The candidate picks one of the jobs they applied to (any status) or a published job they are viewing (`/candidate/interview?job=<id>`, button on the job page). The interviewer receives title, seniority, description, responsibilities, requirements, skills and the English requirement; confidential companies stay unnamed. Generic mode by role family remains for candidates with no applications.
- **Language.** Jobs carry no language field, so the interview language follows the English requirement: B1 or above, English; none, A1 or A2, Spanish. Generic mode lets the candidate choose. Assumption recorded in DECISIONS 92.
- **Transcript integrity.** `mock_interviews.transcript` (jsonb array of `{role, text, at}`), `job_id` and `candidate_turns` are written only by the service role from `sendInterviewMessage`. `guard_mock_interview_update` freezes them for non-admin users; `guard_mock_interview_insert` resets them on insert and rejects a `job_id` the candidate cannot see (`job_not_visible`). Candidate text travels in the `messages` array, never in the system prompt.
- **Feedback.** `finishInterview` (allowed from the fourth reply, automatic when the interviewer closes) grades the transcript (`mock-interview.v2`) in the candidate's UI locale: scores, strengths, improvements, one comment per interviewer question (`q1`, `q2`, …) rendered inline with the transcript, and `language_note` on the candidate's English when the interview was in English. Failed grading can be retried three times; rows from the old runner are still graded from `questions`/`answers`.
- **Unchanged.** Companies never see interview transcripts or scores; admins see role, language and score on the candidate page; the scores inform Dexee's shortlisting only.
