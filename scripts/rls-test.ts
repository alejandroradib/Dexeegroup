...
async function main() {
  ...
  try {
    ...
    const { data: companyA } = await ownerA.client.from("companies").insert({ owner_user_id: ownerA.id, name: `RLS Co A ${suffix}` }).select("id").single();
    const { data: companyB } = await ownerB.client.from("companies").insert({ owner_user_id: ownerB.id, name: `RLS Co B ${suffix}` }).select("id").single();
    ...
    await admin.from("companies").update({ status: "verified", verified_at: new Date().toISOString() }).eq("id", companyA.id);

    const { data: jobA } = await ownerA.client.from("jobs").insert({ company_id: companyA.id, title: "RLS role A", created_by: ownerA.id, status: "published" }).select("id, slug").single();
    const { data: draftA } = await ownerA.client.from("jobs").insert({ company_id: companyA.id, title: "RLS draft A", created_by: ownerA.id }).select("id").single();
    if (!jobA || !draftA) throw new Error("job insert failed");

    const { error: publishPending } = await ownerB.client.from("jobs").insert({ company_id: companyB.id, title: "RLS role B", created_by: ownerB.id, status: "published" });
    record("pending company cannot publish", Boolean(publishPending), publishPending?.message);

    await candidate.client.from("candidates").insert({ id: candidate.id, first_name: "Rls", last_name: "Candidate", data_consent_at: new Date().toISOString(), data_consent_version: "test" });
    await candidate.client.from("candidate_contacts").insert({ candidate_id: candidate.id, email: candidate.email, phone: "+57 300 000 0000" });
    ...
    record("anon cannot read candidates", Boolean(anonCandidates.error) || (anonCandidates.data?.length ?? 0) === 0);
    const anonContacts = await anon.from("candidate_contacts").select("candidate_id");
    record("anon cannot read candidate_contacts", Boolean(anonContacts.error) || (anonContacts.data?.length ?? 0) === 0);

    // Candidate
    const draftApply = await candidate.client.from("applications").insert({ job_id: draftA.id, candidate_id: candidate.id });
    record("candidate cannot apply to a draft job", Boolean(draftApply.error));
    const apply = await candidate.client.from("applications").insert({ job_id: jobA.id, candidate_id: candidate.id }).select("id").single();
    record("candidate applies to a published job", !apply.error, apply.error?.message);
    const dup = await candidate.client.from("applications").insert({ job_id: jobA.id, candidate_id: candidate.id });
    record("candidate cannot apply twice", Boolean(dup.error));
    const ownRows = await candidate.client.from("candidates").select("id");
    record("candidate reads only own candidate row", (ownRows.data?.length ?? 0) === 1 && ownRows.data?.[0]?.id === candidate.id);
    ...
    const releaseByCompany = await ownerA.client.from("applications").update({ contact_released: true }).eq("id", apply.data!.id);
    ...
    const release = await adminClient.from("applications").update({ contact_released: true, contact_released_by: adminUser.id, contact_released_at: new Date().toISOString() }).eq("id", apply.data!.id);
    ...
    const { data: assessment } = await admin.from("assessments").select("id, is_active").eq("type", "english_written").single();
    if (assessment) {
      await admin.from("assessments").update({ is_active: true }).eq("id", assessment.id);
      const first = await candidate.client.from("assessment_attempts").insert({ assessment_id: assessment.id, candidate_id: candidate.id }).select("id").single();
      record("candidate starts an attempt", !first.error, first.error?.message);
      const second = await candidate.client.from("assessment_attempts").insert({ assessment_id: assessment.id, candidate_id: candidate.id });
      record("second in_progress attempt is rejected", Boolean(second.error));
      if (first.data) {
        await admin.from("assessment_attempts").update({ status: "validated", submitted_at: new Date().toISOString(), final_level: "B2" }).eq("id", first.data.id);
        const third = await candidate.client.from("assessment_attempts").insert({ assessment_id: assessment.id, candidate_id: candidate.id });
        record("attempt inside cooldown is rejected", Boolean(third.error) && (third.error?.message ?? "").includes("cooldown_active"));
        cleanup.push(() => admin.from("assessment_attempts").delete().eq("candidate_id", candidate.id));
      }
      await admin.from("assessments").update({ is_active: assessment.is_active }).eq("id", assessment.id);
    }
    ...
    const adminContacts = await adminClient.from("candidate_contacts").select("candidate_id").eq("candidate_id", candidate.id);
