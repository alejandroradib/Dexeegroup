...
import {
  ...
  scoreWorkstyle,
  type LikertItem,
} from "@/lib/assessments/workstyle";
import { workstyleReport } from "@/lib/assessments/workstyle-report";

const items: LikertItem[] = FACTORS.flatMap((factor) =>
  Array.from({ length: 10 }, (_, i) => ({ id: `${factor}-${i}`, factor, reverse: i % 2 === 1 })),
);
...
describe("workstyle scoring", () => {
  ...
  it("builds a deterministic report and picks strengths from the two highest factors", () => {
    const answers = items.map((i) => ({
      question_id: i.id,
      likert_value:
        i.factor === "conscientiousness" || i.factor === "intellect" ? (i.reverse ? 1 : 5) : 3,
    }));
    ...
    const scores = scoreWorkstyle(
      ...
      answers,
      sjt,
      sjt.map((s) => ({ question_id: s.id, selected_option: "b" })),
    );
  ...
  it("omits the SJT strength below the minimum", () => {
    ...
    const scores = scoreWorkstyle(
      ...
      answers,
      sjt,
      sjt.map((s, i) => ({ question_id: s.id, selected_option: i < 6 ? "b" : "a" })),
    );
